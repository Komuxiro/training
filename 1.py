...
days = []
now = datetime.datetime.now()  # здесь храним текущую дату
year = now.year
month = now.month
...
prew_button = Button(window, text='<', command=prew)
...
info_label = Label(window, text='0', width=1, height=1,
                   font=('Verdana', 16, 'bold'), fg='blue')
info_label.grid(row=0, column=1, columnspan=5, sticky='nsew')
# используем функцию fill, которая заполнит наш календарь начальными данными – календарем текущего месяца
for n in range(7):
    lbl = Label(window, text=calendar.day_abbr[n], width=1, height=1,
                font=('Verdana', 10, 'normal'), fg='darkblue')
    lbl.grid(row=1, column=n, sticky='nsew')
for row in range(6):
    for col in range(7):
        lbl = Label(window, text='0', width=4, height=2,
